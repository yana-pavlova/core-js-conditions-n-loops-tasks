...
function rotateMatrix(matrix) {
  ...
  for (let i = 0; i < n; i += 1) {
    for (let j = 0; j < Math.floor(n / 2); j += 1) {
      const temp = matrixRes[i][j];
      matrixRes[i][j] = matrixRes[i][n - 1 - j];
      matrixRes[i][n - 1 - j] = temp;
    }
  }
  ...
  for (let it = 0; it < iterations; it += 1) {
    ...
    if (isOriginal) {
      ...
      for (let j = 0; j < remainingIterations; j += 1) {
        const tempС = new Array(n);
        let evenIndexС = 0;
        let oddIndexС = Math.ceil(n / 2);

        for (let i = 0; i < n; i += 1) {
          if (i % 2 === 0) {
            tempС[evenIndexС] = chars[i];
            evenIndexС += 1;
          } else {
            tempС[oddIndexС] = chars[i];
            oddIndexС += 1;
          }
        }
